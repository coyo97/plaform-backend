...
export class NotificationController {
  ...
  constructor(app: App, route: string, io: SocketIOServer, socketController: SocketController) {
    this.route = route;
    this.app = app;
  ...
  private async createNotification(req: AuthRequest, res: Response): Promise<Response> {
    try {
      ...
      notifications.forEach((notification) => {
        const recipientSocketId = this.socketController.getSocketId(notification.recipient.toString());
        if (recipientSocketId) {
          this.io.to(recipientSocketId).emit('new-notification', notification);
        }
      });
